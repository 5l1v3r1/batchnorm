...
// mulAdd repeats the bias and scale n times and computes
// (in * scale) + bias.
func mulAdd(in, scale, bias autofunc.Result, n int) autofunc.Result {
	outVec := make(linalg.Vector, len(in.Output()))
	copy(outVec, in.Output())
...
func (a *mulAddResult) Output() linalg.Vector {
	return a.OutputVec
}

func (a *mulAddResult) Constant(g autofunc.Gradient) bool {
	return a.Input.Constant(g) && a.Bias.Constant(g) && a.Scale.Constant(g)
}

func (a *mulAddResult) PropagateGradient(upstream linalg.Vector, g autofunc.Gradient) {
	if !a.Scale.Constant(g) {
		downstream := make(linalg.Vector, len(a.Scale.Output()))
		inVec := a.Input.Output()
		for i := 0; i < len(upstream); i += len(downstream) {
			for j := range downstream {
				downstream[j] += upstream[i+j] * inVec[i+j]
			}
		}
		a.Scale.PropagateGradient(downstream, g)
	}
